...
// team name with most points (goal diff breaks ties)
function champions(teams) {
  const points = (t) => 3 * t.wins + t.draws;
  const goalDiff = (t) => t.scored - t.conceded;
  return teams.reduce((best, t) => {
    if (points(t) > points(best)) return t;
    if (points(t) === points(best) && goalDiff(t) > goalDiff(best)) return t;
    return best;
  }).name;
}
