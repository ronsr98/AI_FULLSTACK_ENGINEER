...
// pack whole words into buckets of <= n chars
function bucketize(phrase, n) {
  const words = phrase.split(" ");
  if (words.some((w) => w.length > n)) return []; // a word can never fit
  const buckets = [];
  let current = "";
  for (const w of words) {
    if (current === "") current = w;
    else if ((current + " " + w).length <= n) current += " " + w;
    else { buckets.push(current); current = w; }
  }
