...
// first number by which all digits 0-9 were seen
function findAllDigits(arr) {
  const seen = new Set(); // digits found so far
  for (const num of arr) {
    for (const digit of String(num)) seen.add(digit);
    if (seen.size === 10) return num;
  }
